...
test('home page contract and accessibility', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: '主要导航' })).toBeAttached();
...
test('navigation and empty states remain usable', async ({ page, isMobile }) => {
  await page.goto('./tracks/');
  await expect(page.getByRole('heading', { level: 1, name: '技术指导' })).toBeVisible();
  await expect(page.getByText('尚无已发布的技术方向')).toBeVisible();
  if (isMobile) {
    await page.getByRole('button', { name: '菜单' }).click();
    await expect(page.getByRole('link', { name: '例会' })).toBeVisible();
  }
});
